...
D2.0 adds governed Suggested Project persistence planning. It classifies active
affinity previews into safe staging candidates versus suppressed candidates. The
module only prepares write plans; the script keeps dry-run as the default and
can apply writes only when explicitly requested.
"""
...
class AffinitySummary:
    ...
    def telemetry_lines(self) -> List[str]:
        lines = [
            "=== PROJECT COGNITION — D2.0: SUGGESTED PROJECT PERSISTENCE TELEMETRY ===",
        ...
        lines.append("[Project Cognition] D2 mode: dry_run_default=true; writes=0_unless_apply_flag; project_relation_mutation=disabled; execution_authority_impact=none")
        return lines
