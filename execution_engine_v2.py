...
def _extract_bna_metadata_provenance(item, entries):
    """Build compact provenance for canonical Importance and Urgency."""
    task = item.get("task") or {}
    props = task.get("properties", {}) if isinstance(task, dict) else {}

    current_importance = safe_nested_get(props, "Importance", "select", "name") or ""
    current_urgency = safe_nested_get(props, "Urgency", "select", "name") or ""

    provenance = {
        "importance": {
            "current": current_importance or "unset",
    ...
    # Importance is the canonical execution-value signal.
    for entry in entries:
        props = entry.get("properties", {}) if isinstance(entry, dict) else {}
        reason = _plain_text_property(props.get("Reason", {}))
        parsed = _parse_importance_from_reason(reason)
        if parsed:
            provenance["importance"].update(parsed)
            break

    important_original = _created_original_has_marker(entries, ["important", "high importance"])
    if important_original and provenance["importance"].get("provenance") == "manual_or_unknown":
        provenance["importance"].update({
...
def emit_bna_metadata_provenance_audit(winners):
    ...
    try:
        ...
        for idx, item in enumerate(winners, start=1):
            ...
            print(_format_provenance_line("Importance", provenance["importance"]))
...
def get_defer_until_date(task):
    ...
    return parse_notion_date(get_defer_until_start(task))
