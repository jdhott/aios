...
BACKUP_DIR="$PROJECT_ROOT/backups/metadata_reconciliation_phase2_1_$STAMP"
...
echo "=== AIOS Metadata Reconciliation Phase 2.1 Installer ==="
...
cp "$PACKAGE_DIR/rollback.sh" "$PROJECT_ROOT/tools/rollback_metadata_reconciliation_phase2_1.sh"
...
chmod +x "$PROJECT_ROOT/tools/rollback_metadata_reconciliation_phase2_1.sh"
...
Rollback created by metadata reconciliation phase 2.0 installer.
...
echo "$BACKUP_DIR" > "$PROJECT_ROOT/.metadata_reconciliation_phase2_1_last_backup"
...
echo "grep -E 'METADATA RECONCILIATION|Metadata Reconciliation|Metadata Persistence Guard|Execution rank canonicalization|Canonicalizing Execution Rank|Closed/done|Quick Win deferred cleanup|Closed/done execution cleanup|Mutation error' test_run.log"
