...
# Remove any prior A1.0/A1.1 runtime analytics hook block so the summary is emitted once.
text = re.sub(
    r"\n\s*# === AIOS Runtime Analytics A1\.0 ===\n.*?\n\s*# === END AIOS Runtime Analytics A1\.0 ===\n",
    "\n",
    text,
    flags=re.S,
)
text = re.sub(
    r"\n\s*# === AIOS Runtime Analytics A1\.1 ===\n.*?\n\s*# === END AIOS Runtime Analytics A1\.1 ===\n",
    "\n",
    text,
    flags=re.S,
)
...
hook = '''
    ...
    # Hardened read-only analytics ledger. Writes local CSV/JSON snapshots only.
    ...
    # === END AIOS Runtime Analytics A1.1 ===
'''

if "AIOS Runtime Analytics A1.1" in text and "write_runtime_analytics(ranked, winners" in text:
    print("Runtime analytics A1.1 hook already present; no patch needed.")
...
if needle in text:
    ...
    # Fallback for systems without D1.3: place before persistence updates after BNA printing.
    fallback = "\n    updated = 0\n"
...
path.write_text(text, encoding="utf-8")
print("Patched execution_engine_v2.py with AIOS Runtime Analytics A1.1 hook.")
