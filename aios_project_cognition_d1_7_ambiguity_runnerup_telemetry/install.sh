...
BACKUP_DIR="$TARGET_DIR/.aios_project_cognition_d1_7_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
backup_if_exists "smoke_test_project_cognition_d1_7.sh"
...
cp -p "$PACKAGE_DIR/smoke_test_project_cognition_d1_7.sh" "$TARGET_DIR/smoke_test_project_cognition_d1_7.sh"
chmod +x "$TARGET_DIR/scripts/aios_project_affinity_report.py" "$TARGET_DIR/smoke_test_project_cognition_d1_7.sh"

echo "$BACKUP_DIR" > "$TARGET_DIR/.last_project_cognition_d1_7_backup"
echo "Installed AIOS Project Cognition D1.7 ambiguity runner-up telemetry package."
