...
BACKUP_DIR="$PROJECT_ROOT/backups/metadata_reconciliation_phase1_11_$STAMP"
...
echo "=== AIOS Metadata Reconciliation Phase 1.11 Installer ==="
...
Rollback created by metadata reconciliation phase 1.10 installer.
...
echo "$BACKUP_DIR" > "$PROJECT_ROOT/.metadata_reconciliation_phase1_11_last_backup"
...
echo "grep -E 'METADATA RECONCILIATION|Metadata Reconciliation|Open tasks with Do = Today|Open Best Next Action|Open Best Next Action tasks without|JDI tasks with forbidden|Closed/done|Quick Win deferred cleanup|Closed/done execution cleanup|Mutation error' test_run.log"
