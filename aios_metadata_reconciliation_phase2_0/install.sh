...
BACKUP_DIR="$PROJECT_ROOT/backups/metadata_reconciliation_phase2_0_$STAMP"
...
echo "=== AIOS Metadata Reconciliation Phase 2.0 Installer ==="
...
cp "$PACKAGE_DIR/rollback.sh" "$PROJECT_ROOT/tools/rollback_metadata_reconciliation_phase2.sh"
...
chmod +x "$PROJECT_ROOT/tools/rollback_metadata_reconciliation_phase2.sh"
...
echo "$BACKUP_DIR" > "$PROJECT_ROOT/.metadata_reconciliation_phase2_0_last_backup"
...
echo "grep -E 'METADATA RECONCILIATION|Metadata Reconciliation|Metadata Persistence Guard|Closed/done|Quick Win deferred cleanup|Closed/done execution cleanup|Mutation error' test_run.log"
