...
D2.3 adds canonical preference assisted suppression. It uses read-only
...
class AffinitySummary:
    ...
    def telemetry_lines(self) -> List[str]:
        lines = [
            "=== PROJECT COGNITION — D2.3: CANONICAL PREFERENCE ASSISTED SUPPRESSION ===",
        ...
        lines.append("[Project Cognition] D2.3 mode: dry_run_default=true; writes=0_unless_apply_flag; project_relation_mutation=disabled; execution_authority_impact=none")
        return lines
...
def build_canonical_project_preference_memory(
    ...
    D2.3 does not mutate projects or tasks. It creates a governance-memory preview
...
def _canonical_preference_maps(
    ...
    """Return canonical and absorbed-label lookup maps for D2.3 write guards."""
...
def build_suggested_project_write_plan(
    ...
    *,
    ...
    D2.3 uses canonical preference memory as a suppression aid only. It does not
    ...
    for preview in active_previews:
        ...
        if not base["task_id"]:
            ...
            # D2.3 allows strong canonical preference to dampen overlap noise only
