...
BACKUP_DIR="$PROJECT_ROOT/backups/metadata_reconciliation_phase1_3_$STAMP"
...
echo "=== AIOS Metadata Reconciliation Phase 1.3 Installer ==="
...
# Do not overwrite a richer existing core/__init__.py if present.
...
if grep -q "$MARKER" "$RUN_FILE"; then
  ...
  echo "Installed diagnostics bootstrap into run_aios.py"
fi

cat > "$BACKUP_DIR/ROLLBACK_INSTRUCTIONS.txt" <<EOF
Rollback created by metadata reconciliation phase 1.2 installer.
...
echo "$BACKUP_DIR" > "$PROJECT_ROOT/.metadata_reconciliation_phase1_3_last_backup"
...
echo "grep -E 'METADATA RECONCILIATION|Metadata Reconciliation|Finding detail|Deferred future|Would clear Quick Win' test_run.log"
