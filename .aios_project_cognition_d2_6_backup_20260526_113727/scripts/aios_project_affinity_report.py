...
def runtime_summary_lines(summary: Any, *, applied: int, failed: int, mode: str, max_writes: int, errors: List[str] | None = None) -> List[str]:
    ...
    for pref in list(pref_items or [])[:3]:
        pref_bits.append(f"{pref.get('canonical')}:{float(pref.get('strength', 0.0)):.2f}")
    pref_summary = ", ".join(pref_bits) if pref_bits else "none"

    lines = [
        "=== PROJECT COGNITION — D2.5: RUNTIME OBSERVATION INTEGRATION ===",
        (
        ...
        (
            ...
            f"eligible={len(eligible)}; applied={applied}; failed={failed}; "
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="AIOS D2.5 runtime project cognition summary integration")
    ...
    parser.add_argument("--runtime-summary", action="store_true", help="Emit compact D2.5 runtime summary suitable for cron/test_run logs.")
