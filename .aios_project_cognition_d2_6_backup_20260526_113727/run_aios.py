...
# === AIOS PROJECT COGNITION D2.5 RUNTIME OBSERVATION INTEGRATION ===
def run_project_cognition_runtime_observation():
    ...
    if result.returncode != 0:
        print(f"[Project Cognition Runtime] Observation exited with status={result.returncode}")
        return False
    return True

run_project_cognition_runtime_observation()
# === END AIOS PROJECT COGNITION D2.5 RUNTIME OBSERVATION INTEGRATION ===
...
def get_due_date(task):
    ...
    Supports the older Due property as a compatibility fallback.
    """
    props = task.get("properties", {}) or {}

    for property_name in (DUE_DATE_PROPERTY, "Due"):
...
def is_high_importance(task):
    """Return True when task priority is High Priority."""
    props = task.get("properties", {}) or {}
    return get_select_or_status_name(props, "Priority") == "High Priority"
