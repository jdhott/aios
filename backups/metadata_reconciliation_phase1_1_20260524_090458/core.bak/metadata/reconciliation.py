...
Diagnostics-only reconciliation layer.

This module intentionally does not mutate Notion records. It observes runtime task/page
objects already present in run_aios.py and emits a compact consistency scan for metadata
that should eventually be governed by a canonical reconciliation authority.
"""
...
VERSION = "metadata-reconciliation-phase1-diagnostics-v0.1.0"
_MAX_EXAMPLES = 8
...
def scan_pages(pages: Iterable[Mapping[str, Any]]) -> ReconciliationSummary:
    ...
    for page in pages:
        ...
        deferred_future = _is_future_date(_prop_value(props, ["Defer Until", "Deferred Until", "Defer"] ))
        ...
        if done and has_presentation_metadata:
            summary.finding("done_presentation", "Done tasks with active presentation metadata").add(title)
        if done and has_execution_metadata:
            summary.finding("done_execution", "Done tasks with execution metadata").add(title)
        if jdi and has_execution_metadata:
            summary.finding("jdi_execution", "JDI tasks with forbidden execution metadata").add(title)
        if deferred_future and has_presentation_metadata:
            summary.finding("deferred_surface", "Deferred future tasks still surfaced").add(title)
        if best_next and exec_rank is None:
            summary.finding("bna_without_rank", "Best Next Action without Execution Rank").add(title)
        if best_next and exec_score is None:
            summary.finding("bna_without_score", "Best Next Action without Execution Score").add(title)
        if do_today and not best_next:
            summary.finding("today_without_bna", "Do = Today without Best Next Action").add(title)
        if focus_now:
            summary.finding("legacy_focus", "Legacy Focus/Focus Now metadata still present").add(title)
        if quick_win and done:
            summary.finding("done_quick_win", "Done tasks still marked Quick Win").add(title)
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 1: DIAGNOSTICS ONLY ===")
    ...
    for key in sorted(summary.findings):
        ...
        for ex in f.examples:
            lines.append(f"  - {ex}")
    lines.append("[Metadata Reconciliation] No Notion mutations performed.")
    return lines


def emit_metadata_reconciliation_diagnostics(runtime_globals: Optional[Mapping[str, Any]] = None) -> ReconciliationSummary:
    """Scan available runtime task objects and print diagnostics.

    Safe by design: no API calls, no Notion writes, no ranking changes.
    """
