"""
AIOS Metadata Reconciliation — Phase 2.2 Deterministic Rank Diagnostics + Reconciliation
...
VERSION = "metadata-reconciliation-phase2-deterministic-rank-diagnostics-v0.2.2"
_MAX_EXAMPLES = 8
...
def collect_execution_rank_diagnostics(pages: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    for page in pages:
        ...
        best_next = _bool(_prop_value(props, ["Best Next Action", "Best Next", "BNA"]))
        do_today = _bool(_prop_value(props, ["Do = Today", "Do Today", "Today"]))
        strong_candidate = _bool(_prop_value(props, ["Strong Candidate", "Strong Next Move", "Strong Candidate?"]))
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 2.2: DETERMINISTIC RANK DIAGNOSTICS + CLEANUPS ===")
