"""
AIOS Metadata Reconciliation — Phase 1.7 Meaningful Closed Task Diagnostics

Narrow mutation reconciliation layer plus the next diagnostics-only rule.

Mutates only the already-validated stale Quick Win condition on future-deferred
tasks. Adds preview-only detail for exactly which closed/done task metadata fields
would be cleared in a future mutation package.

Safety boundary: this module only mutates Notion when a page has:
- Defer Until in the future
- Quick Win checked

It does not change execution score, execution rank, Best Next Action, Do = Today,
Focus, evaluator logic, or task content.
"""
...
VERSION = "metadata-reconciliation-phase1-meaningful-closed-task-diagnostics-v0.1.7"
_MAX_EXAMPLES = 8
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 1.7: MEANINGFUL CLOSED TASK DIAGNOSTICS + QUICK WIN CLEANUP ===")
    ...
    lines.append("[Metadata Reconciliation] Non-Quick-Win findings remain diagnostics only.")
    return lines
...
def emit_metadata_reconciliation_diagnostics(runtime_globals: Optional[Mapping[str, Any]] = None) -> ReconciliationSummary:
    ...
    Phase 1.7 behavior:
    - Preview only meaningful closed/done cleanup fields; ignore default Execution Score=0 noise.

    Phase 1.4 mutation boundary:
    - Clear Quick Win only when Defer Until is in the future.
    - Closed/done task metadata findings are diagnostics-only.
