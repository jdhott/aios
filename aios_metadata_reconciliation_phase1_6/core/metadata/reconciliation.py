"""
AIOS Metadata Reconciliation — Phase 1.6 Closed Task Cleanup Preview
...
VERSION = "metadata-reconciliation-phase1-closed-task-cleanup-preview-v0.1.6"
_MAX_EXAMPLES = 8
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 1.6: CLOSED TASK CLEANUP PREVIEW + QUICK WIN CLEANUP ===")
...
def emit_metadata_reconciliation_diagnostics(runtime_globals: Optional[Mapping[str, Any]] = None) -> ReconciliationSummary:
    ...
    Phase 1.6 behavior:
    - Preview closed/done task cleanup fields only; do not mutate them.
