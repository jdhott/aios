"""
AIOS Metadata Reconciliation — Phase 8.0 Governance Anomaly Diagnostics
...
VERSION = "metadata-reconciliation-phase8-governance-anomaly-diagnostics-v0.8.0"
_MAX_EXAMPLES = 8
...
def collect_governance_anomaly_diagnostics(
    ...
    *,
    ...
    This function is intentionally diagnostics-only. It does not decide ranking,
    does not repair metadata, and does not mutate Notion. The goal is to make
    silent authority leakage or stale overlay drift visible in normal cron logs.
    """
...
def format_governance_anomaly_diagnostics(anomalies: Mapping[str, Any]) -> List[str]:
    ...
    lines.append("[Metadata Governance] Anomaly diagnostics are read-only; no ranking or metadata mutation is performed by this layer.")
    return lines
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 8.0: GOVERNANCE ANOMALY DIAGNOSTICS ===")
