"""
AIOS Metadata Reconciliation — Phase 8.2 Governance Observation Baseline

Canonical metadata reconciliation layer backed by a declarative policy registry.
...
- policy-driven stale canonical execution cleanup on closed/done, future-deferred, and JDI tasks
- policy-driven stale presentation overlay cleanup for Quick Win on closed/done, future-deferred, and JDI tasks
...
It does not mutate Do = Today, Focus, Strong Candidate, evaluator logic, or task content.

Phase 8.0 adds compact read-only governance anomaly diagnostics to the default
telemetry surface. It does not expand mutation scope or change execution ranking.
"""
...
try:
    from core.metadata import policy as metadata_policy
except Exception:  # pragma: no cover - keeps standalone smoke tests resilient
    metadata_policy = None  # type: ignore

VERSION = "metadata-reconciliation-phase8-2-governance-observation-baseline-v0.8.2"
_MAX_EXAMPLES = 8
...
def _aliases(field_name: str, fallback: Sequence[str]) -> Sequence[str]:
    """Return aliases from the canonical metadata policy registry when available."""
    if metadata_policy is not None:
        field = getattr(metadata_policy, field_name, None)
        if field is not None and getattr(field, "aliases", None):
            return tuple(field.aliases)
    return fallback


def _policy_status_lines() -> List[str]:
    if metadata_policy is None:
        return ["[Metadata Policy] Registry unavailable; using local fallback aliases"]
    try:
        return list(metadata_policy.policy_status_lines())
    except Exception as exc:
        return [f"[Metadata Policy] Registry status unavailable: {exc}"]


def _task_title_aliases() -> Sequence[str]:
    return _aliases("TASK_TITLE", ["Task Name", "Name", "Title", "Task"])


def _done_aliases() -> Sequence[str]:
    return _aliases("DONE", ["Done", "Complete", "Completed"])


def _open_loop_aliases() -> Sequence[str]:
    return _aliases("OPEN_LOOP", ["Open Loop", "Open", "Active"])


def _jdi_aliases() -> Sequence[str]:
    return _aliases("JDI", ["JDI", "Just Do It", "Just Do It?", "Just Do It"])


def _quick_win_aliases() -> Sequence[str]:
    return _aliases("QUICK_WIN", ["Quick Win", "Quick Wins", "QuickWin"])


def _best_next_aliases() -> Sequence[str]:
    return _aliases("BEST_NEXT_ACTION", ["Best Next Action", "Best Next", "BNA"])


def _defer_until_aliases() -> Sequence[str]:
    return _aliases("DEFER_UNTIL", ["Defer Until", "Deferred Until", "Defer"])


def _execution_score_aliases() -> Sequence[str]:
    return _aliases("EXECUTION_SCORE", ["Execution Score", "Score"])


def _execution_rank_aliases() -> Sequence[str]:
    return _aliases("EXECUTION_RANK", ["Execution Rank", "Rank"])


def _parent_task_aliases() -> Sequence[str]:
    return _aliases("PARENT_TASK", ["Parent Task", "Parent", "Sub-item", "Sub Item"])


def _do_today_aliases() -> Sequence[str]:
    return _aliases("DO_TODAY", ["Do = Today", "Do Today", "Today"])


def _focus_now_aliases() -> Sequence[str]:
    return _aliases("FOCUS_NOW", ["Focus Now", "Focus"])


def _strong_candidate_aliases() -> Sequence[str]:
    return _aliases("STRONG_CANDIDATE", ["Strong Candidate", "StrongCandidate"])
...
def _task_title(page: Mapping[str, Any]) -> str:
    ...
    val = _prop_value(props, _task_title_aliases())
...
def scan_pages(pages: Iterable[Mapping[str, Any]]) -> ReconciliationSummary:
    ...
    for page in pages:
        ...
        done = _bool(_prop_value(props, _done_aliases()))
        open_loop_raw = _prop_value(props, _open_loop_aliases())
        ...
        jdi = _bool(_prop_value(props, _jdi_aliases())) or bool(re.search(r"\b(jdi|just do it)\b", title, re.I))
        quick_win = _bool(_prop_value(props, _quick_win_aliases()))
        best_next = _bool(_prop_value(props, _best_next_aliases()))
        defer_prop = _prop_value(props, _defer_until_aliases())
        defer_text = _date_text(defer_prop).strip()
        deferred_future = _is_future_date(defer_prop)
        exec_score = _number(_prop_value(props, _execution_score_aliases()))
        exec_rank = _number(_prop_value(props, _execution_rank_aliases()))
...
def collect_quick_win_deferred_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, str]]:
    ...
    for page in pages:
        ...
        if not isinstance(props, Mapping):
            continue
        quick_win_prop_name = _prop_name_present(props, _quick_win_aliases())
...
def collect_closed_execution_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for page in pages:
        ...
        exec_score_prop_name = _prop_name_present(props, _execution_score_aliases())
        exec_rank_prop_name = _prop_name_present(props, _execution_rank_aliases())
...
def collect_policy_stale_presentation_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    """Collect policy-driven stale presentation overlay cleanup actions.

    Phase 5 moves Quick Win cleanup from a single special-case deferred cleanup
    into the canonical policy registry boundary. Quick Win remains a passive
    presentation overlay, not execution authority.
    ...
    Manual-only fields such as Do = Today are never mutated here.
    Deprecated legacy fields are ignored.
    """
    actions: List[Dict[str, Any]] = []
    for page in pages:
        if not _looks_like_page(page):
        ...
        reasons: List[str] = []
        if closed_or_done:
            reasons.append("closed_or_done")
        if jdi:
            reasons.append("jdi")
        if deferred_future:
            reasons.append("future_deferred")
        if not reasons:
            continue

        quick_win_prop_name = _prop_name_present(props, _quick_win_aliases())
        quick_win = _bool(props.get(quick_win_prop_name)) if quick_win_prop_name else False
        if quick_win_prop_name and quick_win:
            actions.append({
                "page_id": str(page.get("id") or ""),
                "title": title,
                "properties": {quick_win_prop_name: {"checkbox": False}},
                "reason": "+".join(reasons),
                "detail": "Quick Win=true",
            })
    return [a for a in actions if a["page_id"]]


def apply_policy_stale_presentation_cleanup(actions: Sequence[Mapping[str, Any]]) -> Tuple[int, List[str]]:
    """Apply Phase 5 mutation: clear stale presentation overlay fields."""
    if not actions:
        return 0, []

    token = os.getenv("NOTION_TOKEN", "").strip()
    if not token:
        return 0, ["NOTION_TOKEN missing; policy stale presentation cleanup skipped"]

    try:
        import requests  # type: ignore
    except Exception as exc:
        return 0, [f"requests import failed; policy stale presentation cleanup skipped: {exc}"]
...
def collect_policy_stale_execution_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    """Collect policy-driven stale canonical execution cleanup actions.

    Phase 4 expanded from closed/done score/rank cleanup to the complete
    canonical execution surface declared by the policy registry:
    - Execution Score
    - Execution Rank
    - Best Next Action
    ...
    Manual-only and deprecated legacy fields are not mutated here.
    Quick Win remains governed by the separate deferred-overlay cleanup.
    """
    actions: List[Dict[str, Any]] = []
    for page in pages:
        if not _looks_like_page(page):
        ...
        exec_score_prop_name = _prop_name_present(props, _execution_score_aliases())
        exec_rank_prop_name = _prop_name_present(props, _execution_rank_aliases())
        best_next_prop_name = _prop_name_present(props, _best_next_aliases())
        exec_score = _number(props.get(exec_score_prop_name)) if exec_score_prop_name else None
        exec_rank = _number(props.get(exec_rank_prop_name)) if exec_rank_prop_name else None
        best_next = _bool(props.get(best_next_prop_name)) if best_next_prop_name else False
        ...
        if exec_score_prop_name and exec_score is not None and exec_score != 0:
            properties_to_clear[exec_score_prop_name] = {"number": None}
            detail_parts.append(f"Execution Score={exec_score:g}")
        if best_next_prop_name and best_next:
            properties_to_clear[best_next_prop_name] = {"checkbox": False}
            detail_parts.append("Best Next Action=true")
...
def apply_policy_stale_execution_cleanup(actions: Sequence[Mapping[str, Any]]) -> Tuple[int, List[str]]:
    """Apply Phase 4 mutation: clear stale canonical execution fields."""
    if not actions:
        return 0, []

    token = os.getenv("NOTION_TOKEN", "").strip()
    if not token:
        return 0, ["NOTION_TOKEN missing; policy stale execution cleanup skipped"]

    try:
        import requests  # type: ignore
    except Exception as exc:
        return 0, [f"requests import failed; policy stale execution cleanup skipped: {exc}"]
...
def collect_execution_rank_diagnostics(pages: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    for page in pages:
        ...
        best_next = _bool(_prop_value(props, _best_next_aliases()))
        parentish = bool(_prop_value(props, _parent_task_aliases()))
        rows.append({
            "page_id": page_id,
...
def _verbose_rank_diagnostics_enabled() -> bool:
    """Return True when detailed rank row previews should be printed.

    The compact governance telemetry summary is now the default reconciliation
    observability surface. Set AIOS_VERBOSE_RANK_DIAGNOSTICS=true for targeted
    debugging of persisted-rank and deterministic-order row details.
    """
    return str(os.getenv("AIOS_VERBOSE_RANK_DIAGNOSTICS", "")).strip().lower() in {"1", "true", "yes", "on"}
...
def format_execution_rank_diagnostics(diag: Mapping[str, Any]) -> List[str]:
    ...
    if mismatches:
        ...
        lines.append("[Execution Rank Diagnostics] Rank-order health: clean")

    if not _verbose_rank_diagnostics_enabled():
        lines.append(
            "[Execution Rank Diagnostics] Detailed row previews suppressed "
            "(set AIOS_VERBOSE_RANK_DIAGNOSTICS=true to enable)"
        )
        return lines
...
def collect_deprecated_metadata_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    """Collect narrow cleanup actions for deprecated execution-era metadata.

    Phase 8.1 promoted a validated Phase 8 anomaly signal into a safe cleanup:
    clear checked deprecated legacy boolean fields so they no longer create
    recurring governance attention.

Phase 8.2 adds a final cron-friendly observation baseline line after cleanup
    planning. It does not expand mutation scope. This does not touch canonical execution
    state, manual Do = Today pins, presentation overlays, ranking, scoring, or
    task content.
    """
    actions: List[Dict[str, Any]] = []
    for page in pages:
        ...
        focus_now_prop_name = _prop_name_present(props, _focus_now_aliases())
        if focus_now_prop_name and _bool(props.get(focus_now_prop_name)):
            properties_to_clear[focus_now_prop_name] = {"checkbox": False}
            detail_parts.append(f"{focus_now_prop_name}=true")

        strong_candidate_prop_name = _prop_name_present(props, _strong_candidate_aliases())
        if strong_candidate_prop_name and _bool(props.get(strong_candidate_prop_name)):
            properties_to_clear[strong_candidate_prop_name] = {"checkbox": False}
            detail_parts.append(f"{strong_candidate_prop_name}=true")
        ...
        if properties_to_clear:
            actions.append({
                ...
                "title": title,
                "properties": properties_to_clear,
                "reason": "deprecated_metadata_cleanup",
                "detail": "; ".join(detail_parts),
            })
    return [a for a in actions if a["page_id"]]


def apply_deprecated_metadata_cleanup(actions: Sequence[Mapping[str, Any]]) -> Tuple[int, List[str]]:
    """Apply Phase 8.1 mutation: clear deprecated legacy boolean fields only."""
    if not actions:
        return 0, []

    token = os.getenv("NOTION_TOKEN", "").strip()
    if not token:
        return 0, ["NOTION_TOKEN missing; deprecated metadata cleanup skipped"]

    try:
        import requests  # type: ignore
    except Exception as exc:
        return 0, [f"requests import failed; deprecated metadata cleanup skipped: {exc}"]
...
def collect_governance_anomaly_diagnostics(
    pages: Iterable[Mapping[str, Any]],
    *,
    rank_diag: Optional[Mapping[str, Any]] = None,
) -> Dict[str, Any]:
    """Collect compact read-only governance anomaly counters.

    This function is diagnostics-only. It does not decide ranking or repair
    canonical execution state. Deprecated metadata cleanup is handled by a
    separate narrow reconciliation action below.
    """
    counters: Dict[str, int] = {
        "ranked_without_score": 0,
        "score_without_rank": 0,
        "bna_without_rank_or_score": 0,
        "quickwin_bna_overlap": 0,
        "orphaned_today_flags": 0,
        "deprecated_metadata_seen": 0,
        "future_deferred_surface": 0,
        "closed_or_done_surface": 0,
        "duplicate_execution_ranks": 0,
    }
    examples: Dict[str, List[str]] = {key: [] for key in counters}

    def add(key: str, title: str, detail: str = "") -> None:
        counters[key] += 1
        if len(examples[key]) < _MAX_EXAMPLES:
            examples[key].append(f"{title} — {detail}" if detail else title)
    ...
    for page in pages:
        ...
        open_loop = _bool(open_loop_raw) if open_loop_raw is not None else not done
        closed_or_done = done or (open_loop_raw is not None and not open_loop)
        deferred_future = _is_future_date(_prop_value(props, _defer_until_aliases()))

        quick_win = _bool(_prop_value(props, _quick_win_aliases()))
        best_next = _bool(_prop_value(props, _best_next_aliases()))
        do_today = _bool(_prop_value(props, _do_today_aliases()))
        focus_now = _bool(_prop_value(props, _focus_now_aliases()))
        strong_candidate = _bool(_prop_value(props, _strong_candidate_aliases()))
        exec_score = _number(_prop_value(props, _execution_score_aliases()))
        exec_rank = _number(_prop_value(props, _execution_rank_aliases()))
        meaningful_score = exec_score is not None and exec_score != 0

        if exec_rank is not None and not meaningful_score:
            add("ranked_without_score", title, f"Execution Rank={exec_rank:g}; Execution Score={exec_score}")
        if meaningful_score and exec_rank is None:
            add("score_without_rank", title, f"Execution Score={exec_score:g}; Execution Rank=missing")
        if best_next and (exec_rank is None or not meaningful_score):
            add("bna_without_rank_or_score", title, f"Best Next Action=true; rank={exec_rank}; score={exec_score}")
        if quick_win and best_next:
            add("quickwin_bna_overlap", title, "Quick Win=true; Best Next Action=true")
        if do_today and not best_next:
            add("orphaned_today_flags", title, "Do = Today=true; Best Next Action=false")
        if focus_now or strong_candidate:
            present = []
            if focus_now:
                present.append("Focus Now=true")
            if strong_candidate:
                present.append("Strong Candidate=true")
            add("deprecated_metadata_seen", title, "; ".join(present))

        surfaced = quick_win or best_next or exec_rank is not None or meaningful_score
        if deferred_future and surfaced:
            add("future_deferred_surface", title, _active_surface_details(
                quick_win=quick_win,
                best_next=best_next,
                exec_score=exec_score,
                exec_rank=exec_rank,
                defer_text=_date_text(_prop_value(props, _defer_until_aliases())).strip(),
            ))
        if closed_or_done and surfaced:
            add("closed_or_done_surface", title, _active_surface_details(
                quick_win=quick_win,
                best_next=best_next,
                exec_score=exec_score,
                exec_rank=exec_rank,
    ...
    if rank_diag is not None:
        duplicates = list(rank_diag.get("duplicates") or [])
        counters["duplicate_execution_ranks"] = len(duplicates)
        if duplicates:
            examples["duplicate_execution_ranks"] = [", ".join(str(x) for x in duplicates[:_MAX_EXAMPLES])]

    return {"counters": counters, "examples": examples}


def format_governance_anomaly_diagnostics(anomalies: Mapping[str, Any]) -> List[str]:
    counters = dict(anomalies.get("counters") or {})
    examples = dict(anomalies.get("examples") or {})
    ordered_keys = [
    ...
    total = sum(int(counters.get(key, 0) or 0) for key in ordered_keys)
    lines = [
        "=== GOVERNANCE ANOMALY DIAGNOSTICS ===",
        f"[Metadata Governance] Anomaly signals: total={total}; "
        + "; ".join(f"{key}={int(counters.get(key, 0) or 0)}" for key in ordered_keys),
    ]
    if total:
        for key in ordered_keys:
            ...
            if not count:
                continue
            for ex in list(examples.get(key) or [])[:_MAX_EXAMPLES]:
                lines.append(f"[Metadata Governance] Anomaly detail: {key} — {ex}")
    else:
        lines.append("[Metadata Governance] No governance anomaly signals observed in available runtime objects.")
    lines.append("[Metadata Governance] Anomaly diagnostics do not affect ranking, scoring, BNA selection, or Quick Win behavior.")
    return lines



def format_governance_observation_baseline(
    *,
    anomalies: Mapping[str, Any],
    presentation_stale_actions: Sequence[Mapping[str, Any]],
    policy_stale_actions: Sequence[Mapping[str, Any]],
    deprecated_cleanup_actions: Sequence[Mapping[str, Any]],
    rank_actions: Sequence[Mapping[str, Any]],
    cleanup_error_count: int = 0,
) -> List[str]:
    """Return one final cron-friendly governance observation block.

    Phase 8.2 is intentionally observational. It summarizes whether the current
    runtime is safe to keep observing or whether a human should inspect the fuller
    diagnostics above. It does not mutate metadata and it does not affect ranking,
    scoring, Best Next Action, Quick Win, or Do = Today behavior.
    """
    counters = dict((anomalies or {}).get("counters") or {})
    anomaly_total = sum(int(v or 0) for v in counters.values())
    rank_changes = sum(1 for action in rank_actions if action.get("changed"))
    planned_mutations = (
        len(presentation_stale_actions)
        + len(policy_stale_actions)
        + len(deprecated_cleanup_actions)
        + rank_changes
    )
    if cleanup_error_count:
        status = "attention_required"
    elif anomaly_total or planned_mutations:
        status = "stabilizing"
    else:
        status = "clean"
    ...
    return [
        ...
        (
            "[Metadata Governance] Observation baseline: "
            f"status={status}; anomalies={anomaly_total}; planned_mutations={planned_mutations}; "
            f"cleanup_errors={cleanup_error_count}; observe_next_cron={'yes' if status != 'clean' else 'no'}"
        ),
        (
            "[Metadata Governance] Observation detail: "
            f"stale_presentation={len(presentation_stale_actions)}; "
            f"stale_execution={len(policy_stale_actions)}; "
            f"deprecated_cleanup={len(deprecated_cleanup_actions)}; "
            f"rank_rewrite_changes={rank_changes}"
        ),
        "[Metadata Governance] Observation layer is read-only and does not alter execution authority.",
    ]

def _policy_names(group_name: str, fallback: Sequence[str]) -> str:
    """Return a stable comma-separated field list from the policy registry."""
    if metadata_policy is None:
        return ", ".join(fallback)
    group = getattr(metadata_policy, group_name, None)
    if group:
        try:
            names = [getattr(field, "canonical_name", str(field)) for field in group]
            return ", ".join(names)
        except Exception:
            pass
    return ", ".join(fallback)
...
def _governance_clean_status(
    *,
    summary: ReconciliationSummary,
    presentation_stale_actions: Sequence[Mapping[str, Any]],
    policy_stale_actions: Sequence[Mapping[str, Any]],
    rank_actions: Sequence[Mapping[str, Any]],
    rank_diag: Mapping[str, Any],
) -> str:
    rank_changes = sum(1 for action in rank_actions if action.get("changed"))
    missing = list(rank_diag.get("missing") or [])
    duplicates = list(rank_diag.get("duplicates") or [])
    mismatches = list(rank_diag.get("mismatches") or [])
    if (
        summary.findings
        or presentation_stale_actions
        or policy_stale_actions
        or rank_changes
        or missing
        or duplicates
        or mismatches
...
def format_governance_telemetry_summary(
    *,
    summary: ReconciliationSummary,
    presentation_stale_actions: Sequence[Mapping[str, Any]],
    policy_stale_actions: Sequence[Mapping[str, Any]],
    rank_actions: Sequence[Mapping[str, Any]],
    rank_diag: Mapping[str, Any],
    anomalies: Optional[Mapping[str, Any]] = None,
) -> List[str]:
    """Return a compact governance telemetry block for ordinary run review.

    This is intentionally read-only. It summarizes the reconciliation result after
    the detailed diagnostics have run so a normal log review can confirm whether
    metadata governance is healthy without scanning every diagnostic row.
    """
    findings_count = sum(f.count for f in summary.findings.values())
    active_rank_rows = len(list(rank_diag.get("rows") or []))
    rank_changes = sum(1 for action in rank_actions if action.get("changed"))
    missing = list(rank_diag.get("missing") or [])
    duplicates = list(rank_diag.get("duplicates") or [])
    mismatches = list(rank_diag.get("mismatches") or [])
    anomaly_counters = dict((anomalies or {}).get("counters") or {})
    anomaly_total = sum(int(v or 0) for v in anomaly_counters.values())
    status = _governance_clean_status(
        summary=summary,
        presentation_stale_actions=presentation_stale_actions,
        policy_stale_actions=policy_stale_actions,
        rank_actions=rank_actions,
        rank_diag=rank_diag,
    )
    if anomaly_total and status == "clean":
        status = "attention_required"
    lines = [
        "=== METADATA GOVERNANCE TELEMETRY SUMMARY ===",
        f"[Metadata Governance] Version: {VERSION}",
        (
            "[Metadata Governance] Observed tasks: "
            f"scanned={summary.scanned}; open={summary.open_tasks}; closed_or_done={summary.closed_tasks}; done={summary.done_tasks}"
        ),
        (
            "[Metadata Governance] Reconciliation signals: "
            f"findings={findings_count}; stale_presentation_candidates={len(presentation_stale_actions)}; "
            f"stale_execution_candidates={len(policy_stale_actions)}; active_rank_rows={active_rank_rows}; "
            f"rank_rewrite_changes={rank_changes}"
        ),
        (
            "[Metadata Governance] Rank health: "
            f"missing={missing if missing else 'none'}; duplicates={duplicates if duplicates else 'none'}; "
            f"order_mismatches={len(mismatches)}"
        ),
        (
            "[Metadata Governance] Anomaly health: "
            f"total={anomaly_total}; "
            f"ranked_without_score={anomaly_counters.get('ranked_without_score', 0)}; "
            f"score_without_rank={anomaly_counters.get('score_without_rank', 0)}; "
            f"quickwin_bna_overlap={anomaly_counters.get('quickwin_bna_overlap', 0)}; "
            f"orphaned_today_flags={anomaly_counters.get('orphaned_today_flags', 0)}; "
            f"deprecated_metadata_seen={anomaly_counters.get('deprecated_metadata_seen', 0)}"
        ),
        (
            "[Metadata Governance] Authority boundary: "
            "execution=" + _policy_names("CANONICAL_EXECUTION_FIELDS", ["Execution Score", "Execution Rank", "Best Next Action"])
        ),
        (
            "[Metadata Governance] Overlay/manual/deprecated boundary: "
            "overlays=" + _policy_names("PRESENTATION_OVERLAY_FIELDS", ["Quick Win"]) + "; "
            "manual_only=" + _policy_names("MANUAL_ONLY_FIELDS", ["Do = Today"]) + "; "
            "deprecated_ignored=" + _policy_names("DEPRECATED_EXECUTION_FIELDS", ["Focus Now", "Strong Candidate"])
        ),
        f"[Metadata Governance] Status: {status}",
    ]
    return lines
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 8.2: GOVERNANCE OBSERVATION BASELINE ===")
...
def emit_metadata_reconciliation_diagnostics(runtime_globals: Optional[Mapping[str, Any]] = None) -> ReconciliationSummary:
    ...
    - Clear stale canonical execution metadata from closed/done, future-deferred, and JDI tasks.
    - Clear stale Quick Win overlay metadata from closed/done, future-deferred, and JDI tasks.
    - Full clear-then-rewrite canonical Execution Rank sequence for active ranked rows.
    - Ignore default Execution Score=0 noise.

    Mutation boundary:
    - Clear Quick Win only when stale by policy: closed/done, future-deferred, or JDI.
    ...
    presentation_stale_actions = collect_policy_stale_presentation_cleanup_actions(pages)
    policy_stale_actions = collect_policy_stale_execution_cleanup_actions(pages)
    deprecated_cleanup_actions = collect_deprecated_metadata_cleanup_actions(pages)
    ...
    for line in _policy_status_lines():
        print(line)

    for line in format_execution_rank_diagnostics(rank_diag):
        print(line)

    for line in format_governance_telemetry_summary(
        summary=summary,
        presentation_stale_actions=presentation_stale_actions,
        policy_stale_actions=policy_stale_actions,
        rank_actions=rank_actions,
    ...
    for line in format_governance_anomaly_diagnostics(anomalies):
        print(line)
    ...
    if presentation_stale_actions:
        print(f"[Metadata Reconciliation] Applying policy stale presentation cleanup: {len(presentation_stale_actions)}")
        for action in presentation_stale_actions[:_MAX_EXAMPLES]:
            print(
        ...
        if errors:
            print(f"[Metadata Reconciliation] Policy stale presentation cleanup errors: {len(errors)}")
            for err in errors[:_MAX_EXAMPLES]:
                print(f"[Metadata Reconciliation] Policy stale presentation cleanup error detail: {err}")
    else:
        print("[Metadata Reconciliation] Policy stale presentation cleanup: 0")

    if policy_stale_actions:
        print(f"[Metadata Reconciliation] Applying policy stale execution cleanup: {len(policy_stale_actions)}")
        for action in policy_stale_actions[:_MAX_EXAMPLES]:
            print(
        ...
        cleanup_error_count += len(errors)
        if errors:
            print(f"[Metadata Reconciliation] Policy stale execution cleanup errors: {len(errors)}")
            for err in errors[:_MAX_EXAMPLES]:
                print(f"[Metadata Reconciliation] Policy stale execution cleanup error detail: {err}")
    else:
        print("[Metadata Reconciliation] Policy stale execution cleanup: 0")

    if deprecated_cleanup_actions:
        print(f"[Metadata Reconciliation] Applying deprecated metadata cleanup: {len(deprecated_cleanup_actions)}")
        for action in deprecated_cleanup_actions[:_MAX_EXAMPLES]:
            print(
        ...
        cleanup_error_count += len(errors)
        if errors:
            print(f"[Metadata Reconciliation] Deprecated metadata cleanup errors: {len(errors)}")
            for err in errors[:_MAX_EXAMPLES]:
                print(f"[Metadata Reconciliation] Deprecated metadata cleanup error detail: {err}")
    else:
        print("[Metadata Reconciliation] Deprecated metadata cleanup: 0")
...
#        print(f"[Metadata Reconciliation] Applying true execution rank rewrite: {len(rank_actions)} active rows; changed={changed_count}")
    ...
    for line in format_governance_observation_baseline(
        anomalies=anomalies,
        presentation_stale_actions=presentation_stale_actions,
        policy_stale_actions=policy_stale_actions,
        deprecated_cleanup_actions=deprecated_cleanup_actions,
        rank_actions=rank_actions,
