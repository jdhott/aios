...
A1.2 keeps the CSV lean, writes full forensic detail to NDJSON/JSON, and adds BNA persistence/stability metrics.
"""
...
VERSION = "aios-runtime-analytics-a1.2"
SUMMARY_MARKER = "AIOS RUNTIME ANALYTICS SUMMARY A1.2"
...
def _titles_from_analytics_record(record: Dict[str, Any]) -> List[str]:
    """Return BNA titles from a previous analytics record."""
    titles = record.get("bna_titles")
    if isinstance(titles, list):
        return [str(t) for t in titles if t]
    details = record.get("bna_details")
...
def _read_json_lines(path: Path, limit: int = 250) -> List[Dict[str, Any]]:
    if not path.exists():
        return []
    try:
...
def _read_latest_json(path: Path) -> Optional[Dict[str, Any]]:
    if not path.exists():
        return None
    try:
...
def _consecutive_bna_counts(current_titles: List[str], previous_records: List[Dict[str, Any]]) -> Dict[str, int]:
    """Calculate consecutive appearances for current BNA titles from historical analytics records.

    Counts include the current run. If a title appeared in the immediately previous run and
    the run before that, it receives 3, etc. History is bounded and read-only.
    """
    counts = {title: 1 for title in current_titles if title}
    active = set(counts.keys())
    for record in reversed(previous_records):
        previous_titles = set(_titles_from_analytics_record(record))
        if not previous_titles:
            break
        still_active = active.intersection(previous_titles)
        if not still_active:
            break
        for title in list(still_active):
            counts[title] += 1
        active = still_active
    return counts
...
def write_runtime_analytics(
    ...
    *,
    ...
    details_ndjson = logs_dir / "runtime_analytics_details.ndjson"
    csv_path = logs_dir / "runtime_analytics.csv"

    previous_records = _read_json_lines(details_ndjson)
    previous_latest = _read_latest_json(latest_json)
    if previous_latest:
        previous_records.append(previous_latest)

    current_titles = analytics["bna_titles"]
    consecutive_counts = _consecutive_bna_counts(current_titles, previous_records)
    previous_titles = set(_titles_from_analytics_record(previous_records[-1])) if previous_records else set()
    repeated_from_previous = sum(1 for title in current_titles if title in previous_titles)
    max_consecutive = max(consecutive_counts.values()) if consecutive_counts else 0
    stuck_3plus = sum(1 for title, count in consecutive_counts.items() if count >= 3)

    for detail in analytics.get("bna_details", []):
        title = detail.get("title")
        detail["consecutive_bna_runs"] = consecutive_counts.get(title, 1)
        detail["seen_in_previous_run"] = bool(title in previous_titles)
        prov = detail.get("provenance") or {}
        if isinstance(prov, dict):
            prov["consecutive_bna_runs"] = consecutive_counts.get(title, 1)
            prov["seen_in_previous_run"] = bool(title in previous_titles)

    analytics["bna_repeated_from_previous_count"] = repeated_from_previous
    analytics["bna_max_consecutive_runs"] = max_consecutive
    analytics["bna_stuck_3plus_count"] = stuck_3plus
    analytics["bna_stability"] = [
        {
            "rank": idx,
            "title": title,
            "consecutive_bna_runs": consecutive_counts.get(title, 1),
            "seen_in_previous_run": title in previous_titles,
        }
        for idx, title in enumerate(current_titles, start=1)
    ]

    latest_json.write_text(json.dumps(analytics, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    with details_ndjson.open("a", encoding="utf-8") as detail_fh:
        detail_fh.write(json.dumps(analytics, ensure_ascii=False, sort_keys=True) + "\n")
    ...
    row = {
        ...
        "bna_repeated_from_previous_count": analytics["bna_repeated_from_previous_count"],
        "bna_max_consecutive_runs": analytics["bna_max_consecutive_runs"],
        "bna_stuck_3plus_count": analytics["bna_stuck_3plus_count"],
    ...
    print(f"[Runtime Analytics] details_ndjson: {details_ndjson}")
    print(
    ...
    print(
        "[Runtime Analytics] bna_stability: "
        f"repeated_from_previous={repeated_from_previous}; "
        f"max_consecutive={max_consecutive}; stuck_3plus={stuck_3plus}"
    )
