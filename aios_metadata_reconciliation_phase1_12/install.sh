...
BACKUP_DIR="$PROJECT_ROOT/backups/metadata_reconciliation_phase1_12_$STAMP"
...
echo "=== AIOS Metadata Reconciliation Phase 1.12 Installer ==="
...
Rollback created by metadata reconciliation phase 1.12 installer.
...
echo "$BACKUP_DIR" > "$PROJECT_ROOT/.metadata_reconciliation_phase1_12_last_backup"
...
echo "grep -E 'METADATA RECONCILIATION|Metadata Reconciliation|Open tasks with Do = Today|Open Best Next Action|Open Best Next Action tasks without|JDI tasks with forbidden|Deferred future tasks with|Closed/done|Quick Win deferred cleanup|Closed/done execution cleanup|Mutation error' test_run.log"
