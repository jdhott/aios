...
D2.2 adds canonical project preference memory telemetry. It derives read-only
canonical preference signals from consolidation suggestions, staged stability,
and drift direction so future write-back can dampen ontology fragmentation
without mutating project relations.
"""
...
class AffinitySummary:
    ...
    def telemetry_lines(self) -> List[str]:
        lines = [
            "=== PROJECT COGNITION — D2.2: CANONICAL PROJECT PREFERENCE MEMORY ===",
        ...
        if self.suggested_project_write_plan or self.suggested_project_suppressed:
            lines.append(
                ...
                f"suppressed={len(self.suggested_project_suppressed)}; default=dry_run"
            )
            ...
            lines.append("[Project Cognition] Suggested Project persistence preview: write_candidates=0; suppressed=0; default=dry_run")
        ...
        lines.append("[Project Cognition] D2.2 mode: dry_run_default=true; writes=0_unless_apply_flag; project_relation_mutation=disabled; execution_authority_impact=none")
        return lines
...
def build_canonical_project_preference_memory(
    ...
    D2.2 does not mutate projects or tasks. It creates a governance-memory preview
...
def build_suggested_project_write_plan(
    ...
    *,
    ...
    This never mutates Notion. It prepares a governed staging plan for the script.
    Safe candidates must be high confidence, low/none ambiguity, relation-backed,
    and must not already be flagged as consolidation-risk unless explicitly allowed.
    """
...
def summarize_historical_affinity(
    ...
    *,
    ...
    suggested_project_write_plan, suggested_project_suppressed = build_suggested_project_write_plan(
        active_previews,
