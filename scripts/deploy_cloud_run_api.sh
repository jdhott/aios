...
gcloud run deploy "$SERVICE"   --project "$PROJECT"   --region "$REGION"   --image "$IMAGE"   --no-allow-unauthenticated   --set-env-vars "AIOS_API_ENV=cloudrun"   --set-secrets "SUPABASE_URL=${SUPABASE_URL_SECRET}:latest,SUPABASE_SECRET_KEY=${SUPABASE_SECRET_KEY_SECRET}:latest"   --port 8080
