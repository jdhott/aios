...
def _project_cognition_snapshot_fingerprint(row: Mapping[str, Any]) -> str:
    """Return semantic identity for a Project Cognition snapshot row.

    D2.8.3 intentionally excludes timestamps and run-local counters so stable
    Project Cognition observations are not re-appended every runtime cycle.
    """
    stable = {
...
def _load_project_cognition_snapshot_fingerprints(snapshot_path: Path) -> set[str]:
    """Load semantic fingerprints already present in the local snapshot file."""
        ...
        with snapshot_path.open("r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if isinstance(row, Mapping):
                    fingerprints.add(_project_cognition_snapshot_fingerprint(row))
    except Exception:
        return fingerprints
...
def append_project_cognition_snapshot(summary: Any, *, applied: int, failed: int, mode: str, max_writes: int) -> None:
    """Append passive Project Cognition snapshot rows with semantic deduplication.

    Governance boundaries:
    - writes only local JSONL telemetry under logs/
    - does not mutate Project relations
    - does not affect execution scoring, ranking, BNA, or dashboard state
    """
    ...
    for item in list(summary.active_task_previews or []):
        runner_up = item.get("runner_up") or {}
        rows.append({
            ...
            "runner_up": runner_up.get("project_label", "") if isinstance(runner_up, Mapping) else "",
    ...
    existing_fingerprints = _load_project_cognition_snapshot_fingerprints(snapshot_path)
...
def runtime_summary_lines(summary: Any, *, applied: int, failed: int, mode: str, max_writes: int, errors: List[str] | None = None) -> List[str]:
    """Return compact runtime lines and emit passive D2.8.3 snapshot telemetry."""
    try:
