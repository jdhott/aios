...
def _score_band(score):
    ...
    if score <= 24:
        return "high_10_24"
    return "very_high_25_plus"


def _count_values(items, key_fn):
    counts = {}
    for item in items:
        try:
            key = key_fn(item)
...
def emit_evaluator_tuning_summary(ranked, winners, persisted_ranked):
    """Emit compact read-only evaluator tuning telemetry.

    This report is intentionally observational. It does not change ranking,
    eligibility, persistence, dashboard behavior, Quick Win behavior, or
    metadata reconciliation. Its purpose is to make future evaluator tuning
    empirical by showing what signals are actually selecting BNAs.
    """
    ranked = ranked or []
    winners = winners or []
    persisted_ranked = persisted_ranked or []
    ...
    for item in ranked:
        ...
        if not reasons:
            fallback_counts["zero_score"] += 1
        elif "baseline_executable" in reasons:
            fallback_counts["baseline_executable"] += 1
        elif item.get("score_source") == "legacy_fallback":
            fallback_counts["legacy_fallback"] += 1
        else:
            fallback_counts["primary_evaluator"] += 1
    ...
    for item in winners:
        for reason in item.get("reasons") or []:
            winner_reason_counts[reason] = winner_reason_counts.get(reason, 0) + 1

    score_bands = _count_values(ranked, lambda item: _score_band(item.get("score")))
    winner_bands = _count_values(winners, lambda item: _score_band(item.get("score")))

    print("\n--- Evaluator Tuning Telemetry ---")
    print(
        "[Evaluator Tuning] Pool: "
        f"ranked={len(ranked)}; "
        f"persisted={len(persisted_ranked)}; "
        f"bna={len(winners)}"
    )
    print(
        "[Evaluator Tuning] Score bands: "
        + _format_counts(score_bands)
    )
    print(
        "[Evaluator Tuning] BNA score bands: "
        + _format_counts(winner_bands)
    )
    print(
        "[Evaluator Tuning] Signal distribution: "
        + _format_counts(reason_counts)
    )
    print(
        "[Evaluator Tuning] BNA signal distribution: "
        + _format_counts(winner_reason_counts)
    )
    print(
        "[Evaluator Tuning] Scoring source health: "
        + _format_counts(fallback_counts)
    )
    ...
    if winners:
        ...
        if weak_winners and _verbose_execution_diagnostics_enabled():
            for title in weak_winners[:10]:
                print(
                    "[Evaluator Tuning] Low-signal BNA: "
                    f"{title}"
                )
    ...
    for task in eligible_tasks:
        try:
            ...
            if EVALUATOR_AVAILABLE:
                ...
                score_source = "primary_evaluator"
                ...
                if score == 0:
                    ...
                    if orchestration.legacy_score > 0:
                        ...
                        if structurally_reasonable:
                            ...
                            score_source = "baseline_executable"
