...
def _project_cognition_snapshot_fingerprint(row: Mapping[str, Any]) -> str:
    """Return a stable fingerprint for a passive cognition snapshot row."""
...
def _load_project_cognition_snapshot_state(state_path: Path) -> Dict[str, Any]:
    """Load local snapshot deduplication state. Best-effort and nonfatal."""
...
def _write_project_cognition_snapshot_state(state_path: Path, state: Mapping[str, Any]) -> None:
    """Write local snapshot deduplication state. Best-effort caller handles failures."""
    state_path.write_text(json.dumps(state, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def append_project_cognition_snapshot(summary: Any, *, applied: int, failed: int, mode: str, max_writes: int) -> None:
    """Append passive runtime Project Cognition snapshot rows for ontology observation.

    D2.8 telemetry hygiene:
    - deduplicates identical observations across repeated runtime cycles
    - ignores observed_at when computing fingerprints
    - preserves new drift/change observations

    Governance boundaries:
    - writes only local JSONL telemetry under logs/
    - does not mutate Notion Project relations
    - does not affect execution scoring or ranking
    """
    enabled = os.getenv("AIOS_PROJECT_COGNITION_SNAPSHOT_ENABLED", "true").strip().lower() not in {"0", "false", "no", "off"}
    if not enabled:
        return

    dedup_enabled = os.getenv("AIOS_PROJECT_COGNITION_SNAPSHOT_DEDUP_ENABLED", "true").strip().lower() not in {"0", "false", "no", "off"}
    try:
        max_seen = int(os.getenv("AIOS_PROJECT_COGNITION_SNAPSHOT_DEDUP_MAX_SEEN", "5000") or "5000")
    except Exception:
        max_seen = 5000
    ...
    project_root = Path(__file__).resolve().parents[1]
    snapshot_path = project_root / "logs" / "project_cognition_snapshot.jsonl"
    state_path = project_root / "logs" / "project_cognition_snapshot_state.json"
    ...
    for item in list(summary.active_task_previews or []):
        rows.append({
            "observed_at": observed_at,
            "source": "active_affinity_preview",
            "task_id": item.get("task_id", ""),
            "task_title": item.get("task_title", ""),
            "suggested_project": item.get("project_label", ""),
            "project_key": item.get("project_key", ""),
            "confidence": item.get("confidence", ""),
            "ambiguity": item.get("ambiguity", ""),
            "score": item.get("score", 0),
            "runner_up": (item.get("runner_up") or {}).get("project_label", ""),
            "runner_up_margin": item.get("runner_up_margin"),
            "execution_authority_impact": "none",
            "relation_mutations": 0,
        })

    for item in list((persistence.get("eligible_writes") or [])):
        rows.append({
            "observed_at": observed_at,
            "source": "stability_governed_eligible_write",
            "task_id": item.get("task_id", ""),
            "task_title": item.get("task_title", ""),
    ...
    for item in list(stability.get("drift_items") or []):
        rows.append({
            "observed_at": observed_at,
            "source": "drift_watch",
            "task_id": item.get("task_id", ""),
            "task_title": item.get("task_title", ""),
            "suggested_project": item.get("proposed", ""),
            "existing_suggested_project": item.get("existing", ""),
            "ambiguity": item.get("ambiguity", ""),
            "execution_authority_impact": "none",
            "relation_mutations": 0,
        })
    ...
    if not rows:
        rows.append({
            ...
            "preview_candidates": len(summary.active_task_previews or []),
            "write_candidates": len(summary.suggested_project_write_plan or []),
            "stable_matches": stability.get("stable_matches", 0),
            "execution_authority_impact": "none",
            "relation_mutations": 0,
        })
    ...
    if dedup_enabled:
        state = _load_project_cognition_snapshot_state(state_path)
        seen_list = list(state.get("seen") or [])
        seen = set(seen_list)
        rows_to_write = []

        for row in rows:
            fingerprint = _project_cognition_snapshot_fingerprint(row)
            if fingerprint in seen:
                continue
            rows_to_write.append(row)
            seen.add(fingerprint)
            seen_list.append(fingerprint)
        ...
        state["last_rows_seen"] = len(rows)
        state["last_rows_written"] = len(rows_to_write)
        state["last_rows_deduplicated"] = max(0, len(rows) - len(rows_to_write))
        state["dedup_enabled"] = True
        _write_project_cognition_snapshot_state(state_path, state)
...
def runtime_summary_lines(summary: Any, *, applied: int, failed: int, mode: str, max_writes: int, errors: List[str] | None = None) -> List[str]:
    """Return compact D2.7 runtime lines for normal AIOS logs and snapshot observation."""
    try:
        append_project_cognition_snapshot(summary, applied=applied, failed=failed, mode=mode, max_writes=max_writes)
    except Exception as exc:
        print(f"[Project Cognition Runtime] Snapshot observation skipped: {exc}", file=sys.stderr)
    stability = summary.suggested_project_stability or {}
    persistence = summary.stability_governed_persistence or {}
