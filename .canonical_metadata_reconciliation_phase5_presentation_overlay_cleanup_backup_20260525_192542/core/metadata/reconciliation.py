"""
AIOS Metadata Reconciliation — Phase 4.0 Policy-Driven Stale Execution Cleanup
...
Phase 4.0 makes stale cleanup policy-driven for canonical execution fields.
The only expanded mutation is clearing stale Best Next Action / Execution
Score / Execution Rank from tasks that are closed/done, future-deferred, or JDI.
"""
...
VERSION = "metadata-reconciliation-phase4-policy-driven-stale-execution-cleanup-v0.4.0"
_MAX_EXAMPLES = 8
...
def collect_policy_stale_execution_cleanup_actions(pages: Iterable[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Phase 4 expands from closed/done score/rank cleanup to the complete
...
def format_summary(summary: ReconciliationSummary) -> List[str]:
    lines = []
    lines.append("=== METADATA RECONCILIATION — PHASE 4.0: POLICY-DRIVEN STALE EXECUTION CLEANUP ===")
